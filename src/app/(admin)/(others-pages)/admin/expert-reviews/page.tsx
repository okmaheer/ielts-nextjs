...
export default function AdminExpertReviews() {
  ...
  const [requests, setRequests] = useState<AdminExpertReviewRequest[]>([]);
  ...
  const fetchRequests = async () => {
    try {
      setIsLoading(true);
      const data = await adminExpertReviewService.getAllRequests(filterStatus);
      setRequests(data);
    } catch (err) {
      console.error('Error fetching expert review requests:', err);
      setError('Failed to load expert review requests. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const viewDetails = (requestId: string) => {
    router.push(`/admin/expert-reviews/${requestId}`);
  };
  ...
  if (isLoading) {
    return (
      <div>
        <PageBreadcrumb pageTitle="Expert Review Requests" />
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-gray-600 text-lg">
              Loading expert review requests...
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="px-6 py-4 bg-gradient-to-r from-[#06BBCC]/10 to-transparent border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Click on any request to review and provide feedback
            </p>
          </div>
            ...
            {requests.length === 0 ? (
              <div className="px-6 py-12 text-center">
                ...
                <p className="text-gray-600">
                  {filterStatus === 'all'
                    ? 'No users have requested expert reviews yet'
                    : `No ${filterStatus.replace('_', ' ')} expert review requests at this time`}
                </p>
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        ...
                        {request.submission.expert_score && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-600">Expert Score:</span>
                            <span
                        ...
                        <div className="flex items-center gap-2">
                          <span className="px-2 py-1 bg-gray-100 rounded text-xs">
                            {request.submission.test.category}
                          </span>
                        </div>
                      </div>
                      ...
                      {/* Admin Notes */}
                      {request.admin_notes && (
                        <div className="mt-2 bg-blue-50 p-2 rounded text-sm text-gray-700">
